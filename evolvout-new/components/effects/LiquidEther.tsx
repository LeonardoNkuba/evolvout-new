...
export default function LiquidEther({
  ...
  // Classes precisam estar dentro do useEffect para acessar o escopo do componente
  // (props, refs, etc.) e serem instanciadas durante a montagem
  useEffect(() => {
    ...
    class MouseClass {
      ...
      init(container: HTMLElement) {
        this.container = container;
        // Adicionar listeners com capture phase para garantir que sejam capturados
        // Usar true para capture phase garante que eventos sejam capturados antes de outros elementos
        container.addEventListener('mousemove', this._onMouseMove, true);
        container.addEventListener('touchstart', this._onTouchStart, true);
        container.addEventListener('touchmove', this._onTouchMove, true);
        container.addEventListener('mouseenter', this._onMouseEnter, true);
        container.addEventListener('mouseleave', this._onMouseLeave, true);
        container.addEventListener('touchend', this._onTouchEnd, true);
        
        // Também adicionar no document para garantir captura
      ...
      onDocumentMouseMove(event: MouseEvent) {
        // Verificar se o evento está dentro do container
        if (!this.container) return;
        const rect = this.container.getBoundingClientRect();
        const isInside = event.clientX >= rect.left && event.clientX <= rect.right &&
                         event.clientY >= rect.top && event.clientY <= rect.bottom;
        
        // Se não estiver dentro do container, ignorar (a menos que seja do document)
        if (!isInside && event.target !== document && event.target !== document.body) {
          return;
        }
...
`;
    ...
    class Advection extends ShaderPass {
      ...
      // Override base method with different signature
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      update(...args: any[]) {
    ...
    class ExternalForce extends ShaderPass {
      ...
      update(...args: any[]) {
        const props = args[0];
        if (!this.mouse || !this.mouse.material) return;
    ...
    class Simulation {
      ...
      createAllFBO() {
        ...
        for (const key in this.fbos) {
          const typedKey = key as keyof typeof this.fbos;
          this.fbos[typedKey] = new THREE.WebGLRenderTarget(this.fboSize.x, this.fboSize.y, opts);
        }
      }
      createShaderPass() {
    ...
    class Output {
      ...
      init() {
        ...
        this.scene = new THREE.Scene();
        this.camera = new THREE.Camera();
        const velTexture = this.simulation.fbos.vel_0?.texture;
        if (!velTexture) return;
        ...
        if (this.scene) this.scene.add(this.output);
      }
    ...
    class WebGLManager {
      ...
      init() {
        ...
        // Garantir que o canvas receba eventos
